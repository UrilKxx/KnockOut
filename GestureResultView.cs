...
namespace KnockOut
{
    ...
    public sealed class GestureResultView : INotifyPropertyChanged
    {
        ...
        private readonly ImageSource startImage = new BitmapImage(new Uri(@"Images\Wave.jpg", UriKind.Relative));

        /// <summary> Image to show when the 'detected' property is false for a tracked body </summary>
        private readonly ImageSource notStartImage = new BitmapImage(new Uri(@"Images\notHand.jpg", UriKind.Relative));
        ...
        public GestureResultView(int bodyIndex, bool isTracked, bool detected, float confidence, String gesture="")
        {
            ...
            private set
            {
                if (this.confidence != value)
                {
                    ...
                    //if(this.confidence!=0)
                        //System.Windows.MessageBox.Show("Conf = {0}" + this.confidence);
                    this.NotifyPropertyChanged();
                }
            }
        }
        ...
        public void UpdateGestureResult(bool isBodyTrackingIdValid, bool isGestureDetected, float detectionConfidence, String gesture)
        {
            ...
            /*if(confidence>0.8)
            {
                if (gesture == "Wave")
                    MainMenu.bWave = true;
                else if (gesture == "Stance")
                    MainMenu.bStance = true;
                else if (gesture == "Quit")
                    MainMenu.bQuit = true;
            }
            else
            {
                MainMenu.bQuit = false;
                MainMenu.bStance = false;
                MainMenu.bWave = false;
            }*/
        }
